...
import { OPENAI_KEY } from '../constants/global.js';
...
class MarkdownEmbeddingSource extends BaseEmbeddingSource {
  ...
  constructor(
    ...
    const pattern = new RegExp(`^${repoDir}`, '');
...
async function generateEmbeddings(pathToProcess: string, trainingGroupId: string, repoDir: string) {
  const embeddingSources: EmbeddingSource[] = [
    ...(await walk(pathToProcess))
      .filter(({ path }) => /\.mdx?$/.test(path))
      .filter(({ path }) => !ignoredFiles.includes(path))
      .map((entry) => new MarkdownEmbeddingSource('guide', entry.path, repoDir)),
  ];
